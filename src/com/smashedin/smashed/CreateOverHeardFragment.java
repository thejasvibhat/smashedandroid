...
public class CreateOverHeardFragment extends Fragment implements OnResponseListener {
	...
	public void UploadActual(String mode) throws Exception
	{
	    ...
	    int oHeight = (int) (Singleton.getInstance().dipToPixels(getActivity(), 400)/oData.mThumbIds.size());
	    root.setAttribute("width", String.valueOf(Singleton.getInstance().dipToPixels(getActivity(), 350)));
	    root.setAttribute("height", String.valueOf(Singleton.getInstance().dipToPixels(getActivity(), 400)));
	...
	private void ShareOverheard()
	{
        ...
        int totalHeight = mGridView.getHeight();
        int totalWidth = mGridView.getWidth();
        mGridView.layout(0, 0, totalWidth, totalHeight);    
